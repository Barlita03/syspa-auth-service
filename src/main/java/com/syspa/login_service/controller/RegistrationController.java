...
public class RegistrationController {
  ...
  public ResponseEntity<?> createUser(
    ...
    auditLogger.info(
        "event=USER_REGISTERED, username={}, email={}, id={}",
  ...
  @PostMapping("login")
  public ResponseEntity<?> logUser(@RequestBody @jakarta.validation.Valid LoginRequest request) {
    if (request.getRecaptchaToken() == null || request.getRecaptchaToken().isBlank()) {
      return new ResponseEntity<>("Missing reCAPTCHA token", HttpStatus.BAD_REQUEST);
    }
    if (!recaptchaService.verify(request.getRecaptchaToken())) {
    ...
    auditLogger.info(
        "event=USER_LOGIN, username={}, id={}, role={}",
        dbUser.getUsername(),
        dbUser.getId(),
        dbUser.getRole());
  ...
  public ResponseEntity<?> refreshToken(@RequestBody RefreshRequest request) {
    ...
    auditLogger.info(
        "event=REFRESH_TOKEN_ROTATED, username={}, oldToken={}, newToken={}",
        username,
        oldToken.getToken(),
